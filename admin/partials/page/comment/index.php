...
if (!class_exists('Npcink_Page_Comment')) {
    class Npcink_Page_Comment
    {
        public static function run($option)
        {
            ...
            //评论区添加表情
            $comment_emote = MaBox_Admin::get_config($option, 'comment_emote');
            ...
            //评论时间间隔 - 失效
            $interval = MaBox_Admin::get_config($option, 'interval');
            if ($interval === true) {
            ...
            //评论最少和最多字数
            $words_number = MaBox_Admin::get_config($option, 'words_number');
            ...
            //禁止纯英文评论
            $english = MaBox_Admin::get_config($option, 'english');
            if ($english === true) {
            ...
            //一篇文章只能评论一次
            $only = MaBox_Admin::get_config($option, 'only');
            if ($only === true) {
            ...
            //修改评论区样式中的管理员信息
            $modify_comment_user = MaBox_Admin::get_config($option, 'modify_comment_user');
