...
class ModuleLoaderRuntimeContractTest extends TestCase {
    ...
    protected function setUp(): void {
        ...
        require_once $plugin_dir . '/includes/interface-npcink-toolbox-module.php';
        require_once $plugin_dir . '/tests/fixtures/modules/loader-contract-modules.php';

        $reflection = new ReflectionClass('Npcink_Toolbox_Module_Loader');
        ...
        Npcink_Toolbox_Loader_Contract_Test_Module::$argument_count = null;
        Npcink_Toolbox_Loader_Contract_Test_Module::$received_config = null;
        Npcink_Toolbox_Loader_Non_Interface_Test_Module::$did_run = false;
    }
    ...
    public function test_loader_passes_an_empty_array_without_a_config_path(): void {
        $this->set_test_registry('Npcink_Toolbox_Loader_Contract_Test_Module');

        Npcink_Toolbox_Module_Loader::load_module('test.contract', array('ignored' => true));

        $this->assertSame(1, Npcink_Toolbox_Loader_Contract_Test_Module::$argument_count);
        $this->assertSame(array(), Npcink_Toolbox_Loader_Contract_Test_Module::$received_config);
    }

    public function test_loader_passes_the_resolved_configuration_subtree(): void {
        $this->set_test_registry('Npcink_Toolbox_Loader_Contract_Test_Module', 'settings.module');
        $expected = array('enabled' => true, 'mode' => 'strict');

        Npcink_Toolbox_Module_Loader::load_module(
            'test.contract',
            array('settings' => array('module' => $expected))
        );

        $this->assertSame(1, Npcink_Toolbox_Loader_Contract_Test_Module::$argument_count);
        $this->assertSame($expected, Npcink_Toolbox_Loader_Contract_Test_Module::$received_config);
    }

    public function test_loader_normalizes_missing_or_non_array_configuration_to_an_empty_array(): void {
        $this->set_test_registry('Npcink_Toolbox_Loader_Contract_Test_Module', 'settings.module');

        Npcink_Toolbox_Module_Loader::load_module('test.contract', array());
        $this->assertSame(array(), Npcink_Toolbox_Loader_Contract_Test_Module::$received_config);
    ...
    public function test_loader_refuses_a_module_that_does_not_implement_the_interface(): void {
        $this->set_test_registry('Npcink_Toolbox_Loader_Non_Interface_Test_Module');

        Npcink_Toolbox_Module_Loader::load_module('test.contract', array());

        $this->assertFalse(Npcink_Toolbox_Loader_Non_Interface_Test_Module::$did_run);
    }
    ...
    public function test_loader_keeps_structured_audit_without_error_log_fallback(): void {
        ...
        $this->assertStringContainsString('Npcink_Toolbox_Audit_Logger::log(', $source);
    ...
    public function test_loader_keeps_single_option_key_activation_semantics(): void {
        ...
        $this->assertSame(
            array('test.contract'),
            Npcink_Toolbox_Module_Loader::get_active_modules(array('settings' => array('enabled' => true)))
        );
        $this->assertSame(
            array(),
            Npcink_Toolbox_Module_Loader::get_active_modules(array('settings' => array('enabled' => false)))
        );
        $this->assertSame(
            array(),
            Npcink_Toolbox_Module_Loader::get_active_modules(array('settings' => array('enabled' => 'false')))
    ...
    public function test_activation_paths_use_explicit_any_of_semantics_without_duplicates(): void {
        $this->set_test_registry(
            'Npcink_Toolbox_Loader_Contract_Test_Module',
            null,
            array(
