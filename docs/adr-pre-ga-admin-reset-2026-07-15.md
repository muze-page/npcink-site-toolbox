...
| 项目 | 决定 |
| --- | --- |
| 失败证据 | `function.config` 的 PHP 模块没有注册 Hook、输出或其他运行行为；Admin 又把 Schema 中声明为 `number` 的 `tips_time` 接到时间段控件，提交值为数组，严格设置校验会拒绝保存 |
| 决定 | 在没有用户和兼容负担的 3.0.1 阶段完整移除该模块的 Registry、Schema、Admin 和加载入口，不为无效设置保留迁移器、兼容层或占位 UI |
| 当前事实 | Registry 为 55 个运行时条目（18/18/10/4/5）；用户可理解的功能/任务入口为 58 项，其中站点优化 8 项、辅助功能 5 项 |
| 历史边界 | 工作包 14B 和 3.0.0 更新记录中的 57 个 Registry 模块、60 项用户功能属于当时验收快照，继续保留原文，不改写为当前值 |
| 验收边界 | 文档门禁核对当前计数、公式与历史快照；代码门禁必须另行验证 Registry/Schema/生成契约/Admin 无残留、相关自动化测试与构建通过，并在 Local WordPress 确认入口消失且既有设置仍可读取和保存 |
| 回滚计划 | 若清退导致既有设置面不可用，则整体回滚工作包 15；不恢复只有前端表单而没有运行逻辑的半成品状态 |
